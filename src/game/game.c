...
static int game_check_disk_space();
...
static int game_check_disk_space()
{
    unsigned int min_free_space = 20480;
    unsigned long current_free_space;
    char error[256];

    config_get_value(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_FREE_SPACE_KEY, &min_free_space);

    if (GetFreeDiskSpace(&current_free_space) == -1) {
        current_free_space = min_free_space;
    }

    if (current_free_space < min_free_space) {
        sprintf(error, "Not enough free hard disk space.  Fallout requires at least %.1f megabytes of free hard disk space.", (float)min_free_space / 1024.0f);
        GNWSystemError(error);
        return -1;
    }
